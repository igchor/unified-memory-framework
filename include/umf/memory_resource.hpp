...
class disjoint_pool_resource : public detail::pool_resource {
  public:
    using options = umf_disjoint_pool_params_t;
...
class jemalloc_pool_resource : public detail::pool_resource {
  public:
    using options = umf_jemalloc_pool_params_t;
    jemalloc_pool_resource(std::pmr::memory_resource *upstream,
                           const options &opts = umf_jemalloc_pool_params_t{false})
        : detail::pool_resource(upstream, &UMF_JEMALLOC_POOL_OPS,
                                (void *)&opts) {}
};
} // namespace umf
