 ...
 * It is used to allocate memory for OS specific structures used during loading and starting
 * applications (for example _dl_init() and _dl_fini() on Linux storing arrays of all
 * constructors and destructors that have to be called) and to allocate memory needed
 * by umfMemoryProviderCreate() and umfPoolCreate().
 * That memory will be leaked on purpose (we believe OS will free it),
 * because we cannot free the memory containing array of destructors
 * that have to be called at the end.
 * It uses 892 kB on Linux.
 *
 * Ad 2)
