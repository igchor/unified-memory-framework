...
TEST_P(umfPoolWithCreateFlagsTest, memoryPoolTrace) {
    ...
    auto proxyPool = wrapPoolUnique(
        createPoolChecked(&UMF_PROXY_POOL_OPS, provider, nullptr, flags));
    ...
    // TODO: calloc is not supported by proxy pool
    // TODO: realloc is not supported by proxy pool
...
TEST_F(test, retrieveMemoryProvider) {
    ...
    auto pool = wrapPoolUnique(
        createPoolChecked(&UMF_PROXY_POOL_OPS, provider, nullptr));
...
TEST_P(umfPoolWithCreateFlagsTest, umfPoolCreateFlagsNullPoolHandle) {
    ...
    ret = umfPoolCreate(&UMF_PROXY_POOL_OPS, provider, nullptr, flags, nullptr);
    ASSERT_EQ(ret, UMF_RESULT_ERROR_INVALID_ARGUMENT);
...
TEST_F(test, getLastFailedMemoryProvider) {
    ...
    auto pool = wrapPoolUnique(
        createPoolChecked(&UMF_PROXY_POOL_OPS, hProvider, nullptr));
